...
cd ~/.emacs.d
git init
git remote add origin git@github.com:acarrab/portableEmacs.git
git pull origin master
git reset --hard origin/master
   #+END_SRC

You can also define your own commands separate from given ones by
defining them in a file called ~user.org~ that could be formatted
similarly to my ~loader.org~. The file ~user.org~ will be
automatically loaded in ~init.el~ file.
